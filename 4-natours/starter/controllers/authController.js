...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!token) {
    return next(
      new AppError('Your are not logged in. Please log in for access', 401)
    );
  }
  ...
  // 4) Check if uuser changed password after token was issued.
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    return next(
      new AppErrer('there was an error sending the email. Try again later', 500)
    );
  }
});
